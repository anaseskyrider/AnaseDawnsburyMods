        ...
        ModData.ItemNames.CastersTarge = ModManager.RegisterNewItemIntoTheShop(
            ...
            iName =>
            {
                ...
                Item targe = new Item(
                    ...
                    .WithDescription(
                        "This small shield is made from wood. It features a special panel along the inside surface that holds scrolls.",
                        $"You can store {scrollLimit} spell {S.PluralizeIf("scroll", scrollLimit)} in this shield {{i}}(out-of-combat, drag a scroll onto this shield to store it){{/i}}. You must have a free hand to activate a scroll, and can only do so once per encounter {{i}}(this makes the other scrolls inaccessible){{i}}.")
                    .WithWeaponProperties(new WeaponProperties("1d6", DamageKind.Bludgeoning))
